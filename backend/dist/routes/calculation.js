...
router.get('/typologies', async (req, res) => {
    try {
        ...
        const where = productLineId ? { productLineId: productLineId } : {};
...
router.post('/calculate', async (req, res) => {
    try {
        ...
        // Calcular
        const result = await calculation_1.calculationEngine.calculate({ typologyId, productLineId, width, height, quantity, options }, {
            typology,
...
router.post('/calculate/optimize', async (req, res) => {
    try {
        ...
        const result = calculation_1.calculationEngine.optimizeCut(requiredLengths);
        res.json(result);
    }
    catch (error) {
...
router.post('/calculate/quote', async (req, res) => {
    try {
        ...
        for (const item of items) {
            ...
            const result = await calculation_1.calculationEngine.calculate({ ...item, typologyId: item.typologyId }, { typology, profiles: typology.profiles, accessories: typology.accessories, glasses });
            calculations.push({
                ...result,
                width: item.width,
                height: item.height,
...
router.post('/seed', async (req, res) => {
    try {
        ...
        // Associar perfis à tipologia (fórmulas simplificadas)
        await Promise.all([
            ...
            prisma.typologyProfile.create({
                data: { typologyId: jc2f.id, profileId: profiles[2].id, formula: 'height * 2', cutMargin: 20 } // 2 Verticais externos = altura * 2
            }),
            prisma.typologyProfile.create({
                data: { typologyId: jc2f.id, profileId: profiles[4].id, formula: 'height * 1', cutMargin: 20 } // 1 Intermediário = altura
            }),
            prisma.typologyProfile.create({
        ...
        await Promise.all([
            ...
            prisma.typologyAccessory.create({
                data: { typologyId: jc2f.id, accessoryId: accessories[3].id, formula: '(width + height) * 2', minQty: 1 } // Escova perimetral
            })
        ]);
